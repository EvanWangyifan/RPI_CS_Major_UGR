...
class ParticleFilter(InferenceModule):
	...
	def initializeUniformly(self, gameState):
		...
		for i in range(self.numParticles):
			self.l_particle.append(self.legalPositions[i % len(self.legalPositions)])
	...
	def observe(self, observation, gameState):
		...
		if noisyDistance is not None:
			for p in self.l_particle:
				beliefs[p] += emissionModel[util.manhattanDistance(p, pacmanPosition)]
			if beliefs.totalCount() != 0:
				l_p = []
				for i in range(len(self.l_particle)):
					l_p.append(util.sample(beliefs))
				self.l_particle = l_p
			else:
				self.initializeUniformly(gameState)
		else:  # ghost captured
			for i in range(len(self.l_particle)):
				self.l_particle[i] = self.getJailPosition()
	...
	def elapseTime(self, gameState):
		...
		l_p = []
		for p in self.l_particle:
			l_p.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, p))))
		self.l_particle = l_p
