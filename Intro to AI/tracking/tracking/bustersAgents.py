...
class GreedyBustersAgent(BustersAgent):
	...
	def chooseAction(self, gameState):
		...
		for dist in livingGhostPositionDistributions:
			l_dist = sorted(dist.items(), key=lambda x: x[1])
			bestPos = l_dist[-1][0]
			l_bestPos.append((self.distancer.getDistance(pacmanPosition, bestPos), bestPos))
		mostLikelyPos = sorted(l_bestPos)[0][1]
		
		l_actions = []
		for a in legal:
			l_actions.append((self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), mostLikelyPos), a))
		return sorted(l_actions)[0][1]
