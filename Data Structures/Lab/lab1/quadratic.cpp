...
int main() {
  ...
  int array_b[5], array_c[5];
  const int my_a = 1;
  for (int i=0; i<5; i++)
  {
    std::cout << "Enter 5 b" << std::endl;
    int my_b;
    std::cin >> my_b;
    array_b[i] = my_b;
  }
  ...
  for (int i=0; i<5; i++)
  {
    ...
    float root_1, root_2;
    bool success = find_roots(my_a,array_b[i],array_c[i], root_1,root_2);
    ...
    // Check our work...
    if (check_root(my_a,array_b[i],array_c[i], root_1) && check_root(my_a,array_b[i],array_c[i], root_2)) {
      // Verified roots, break out of the while loop
      if (root_1 < root_2)
      {
        small_roots[i] = root_1;
      } 
      else
      {
        small_roots[i] = root_2;
      }
  ...
  for (int i=0; i<5; i++)
  {
    std::cout << small_roots[i] << std::endl;
  }
