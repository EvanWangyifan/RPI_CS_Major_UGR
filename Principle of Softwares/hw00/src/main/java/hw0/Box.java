...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        ...
        else if (this.getVolume()+b.getVolume()>max){
            return false;
        }
        else{
            ballContainer.add(b);
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        // Your code goes here.  Remove the exception after you're done.
        List<Ball> all_balls = new ArrayList<Ball>();
        Iterator<Ball> itr = this.iterator();
        while (true){
            if (itr.hasNext()) {
                all_balls.add(itr.next());
            }
            else {break;}
        }
        all_balls.sort(Comparator.comparing(Ball::getVolume));
        return all_balls.iterator();
    }
