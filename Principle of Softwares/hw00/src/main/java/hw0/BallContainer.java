...
public class BallContainer implements Iterable<Ball> {
     ...
     */
    public BallContainer() { contents = new LinkedHashSet<Ball>(); }
    ...
    public int differentColors() {
        // Your code goes here.  Remove the exception after you're done.
        Set<Color> all_colors = new LinkedHashSet<Color>();
        for(Ball b: contents){
            all_colors.add(b.getColor());
        }
    ...
    public boolean areSameColor() {
        ...
        if (all_colors.size() == 1 || contents.size() == 0) {
            return true;
        } else {
            return false;
        }
