...
public final class RatPoly {
    ...
    public RatNum getCoeff(int pow) {
        if ((this == ZERO) || (pow < 0) || (pow >= coeffs.length)){
            return new RatNum(0);
        }
        else{
    ...
    public RatPoly negate() {
        RatNum[] arr = new RatNum[this.coeffs.length];
        if (this.isNaN()){
            return NaN;
        }
        if (this.degree==0){
        ...
        for (int i=0; i<this.coeffs.length; i++){
            arr[i] = this.coeffs[i].negate();
        }
        RatPoly result = new RatPoly(arr);
        return result;
    }
    ...
    public RatPoly add(RatPoly p) {
        ...
        if (p.degree > this.degree){
            arr = new RatNum[p.degree+1];
            max_len = p.degree+1;
        }
        else{
            arr = new RatNum[this.degree+1];
            max_len = this.degree+1;
        }
        for (int i=0; i<max_len; i++){
            if (i>p.degree){
                ...
                arr[i] = this.coeffs[i].add(p.coeffs[i]);
            }
        }
        if (arr[arr.length-1].equals(RatNum.ZERO)){
            return ZERO;
        }
    ...
    public RatPoly sub(RatPoly p) {
        ...
        if (this.equals(p) || (this.equals(ZERO) && p.equals(ZERO))){
            return ZERO;
        }
        else if(this.equals(ZERO) && !p.equals(ZERO)){
            return p.negate();
        ...
        for (int i=0; i<max_len; i++){
            if (i>p.degree){
                ...
                arr[i] = this.coeffs[i].sub(p.coeffs[i]);
            }
        }
        return new RatPoly(arr);
    }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        RatNum[] arr = new RatNum[this.degree+p.degree+1];
        ...
        for (int i=0; i<this.degree+1; i++){
            for (int j=0; j<p.degree+1; j++){
                RatNum temp = this.coeffs[i].mul(p.coeffs[j]);
                arr[i+j] = arr[i+j].add(temp);
            }
        }
        // Return
        RatPoly r = new RatPoly(arr);
        checkRep();
        return r;
    }
    ...
    public RatPoly div(RatPoly p) {
        ...
        if (this.equals(ZERO) || (p.degree > this.degree)){
            return ZERO;
        }
        RatNum[] arr = new RatNum[this.degree-p.degree+1];
        RatNum[] remainder = new RatNum[this.degree+1];
        for (int i=0; i<this.degree+1; i++){
            remainder[i] = this.coeffs[i];
        }

        int index = 0;
        int degree_diff = this.degree - p.degree;
        while (degree_diff>=0){
            arr[degree_diff] = remainder[this.degree-index].div(p.coeffs[p.degree]);
            for (int i=this.degree()-p.degree-index; i<this.coeffs.length - index; i++){
                int temp = i + 1 + index + p.degree - remainder.length;
                remainder[i] = remainder[i].sub(arr[degree_diff].mul(p.coeffs[temp]));
            }
            degree_diff--;
            index++;
        }
    ...
    public double eval(double d) {
        ...
        double result = 0;
        for (int i=0; i<this.coeffs.length; i++){
            double temp = Math.pow(d,i);
            temp *= this.coeffs[i].doubleValue();
            result += temp;
        }
        checkRep();
        return result;
    ...
    public RatPoly differentiate() {
        ...
        RatNum[] arr = new RatNum[this.degree];
        for (int i=0; i<this.degree; i++){
            arr[i] = new RatNum(this.coeffs[i+1].intValue()*(i+1));
        }
        RatPoly result = new RatPoly(arr);
        checkRep();
        return result;
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        if(this.equals(ZERO) && integrationConstant == RatNum.ZERO){
            return ZERO;
        }
        if(this.equals(ZERO) && integrationConstant != RatNum.ZERO){
        ...
        RatNum[] arr = new RatNum[this.degree+2];
        for(int i=this.degree+1; i>0; i--){
            arr[i] = new RatNum(this.coeffs[i-1].intValue(), i);
        }
        arr[0] = integrationConstant;
        RatPoly q = new RatPoly(arr);
        return q;
    }
    ...
    public double integrate(double lowerBound, double upperBound) {
        ...
        RatPoly result = antiDifferentiate(new RatNum(0));
        double r = result.eval(upperBound) - result.eval(lowerBound);
        return r;
    }
