...
uint32_t get_index( uint32_t address, uint32_t idx_start, uint32_t idx_end )
{
  ...
  /* YOUR CODE HERE */
  uint32_t tmp = address << (31 - idx_end);
  tmp = tmp >> (31-idx_end + idx_start);
  return tmp;
...
uint32_t get_cache_block_size( uint32_t idx_start, uint32_t idx_end, uint32_t bob )
{
  ...
  /* YOUR CODE HERE */
  uint32_t tag = 32 - idx_end;
  uint32_t box_size = (1<<idx_start)*8+tag;
  return box_size;
}
...
int main()
{
  ...
  printf( "get_cache_block_size( 4, 15, 3 ) returned %d (should be 145)\n",
