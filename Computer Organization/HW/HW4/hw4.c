	  	...
	  	// Not an assignment
		  	...
		  	while (line[line_index][var_index1+1]!=';'){
				...
				// If it is multiplication or div
									...
									if (binary[k]==1){
										...
										for (i=0;i<array_index;i++){ // find the right s
							  			...
							  			if (first_mult==0){ // use move
							  				...
							  				printf("add $t%d,$t%d,$t%d\n",temp_index+1,temp_index+1,temp_index);
							  			}
							  			first_mult = 1;// change flag
									}
								}
								// Add
								printf("add $t%d,$t%d,$s%d\n",temp_index+1,temp_index+1,i);
						...
						// Constant Division
							...
							// Normal division
								...
								if (temp_var%2==0){
									...
									printf("bltz ");
									if (line[line_index][temp_sign_index-4]!='*'){
										for (i=0;i<array_index;i++){ // find the right s
							  				if (all_var_c[i]==line[line_index][var_index1]){
							  					printf("$s%d,",i);
							  					break;
			  	...
			  	// Reuse temp register if it reaches 9
			  	if (temp_index==10){
			  		temp_index = 0;
			  	}
			  	int i;
